#### FILE UPLOAD VERSION ####
pacman::p_load("shiny","shinydashboard","shiny","shinydashboard",
               "shinydashboardPlus","dashboardthemes","stringr","data.table",
               "janitor","tidyverse","flexdashboard","lubridate",
               "htmltools","DT","wesanderson","MetBrewer",
               "knitr","shades","ggrepel","scales",
  ...
  sidebar = dashboardSidebar(minified = TRUE,
                             ...
                             sidebarMenu(
                               menuItem("Inputs", icon=icon("cogs"), startExpanded = TRUE, selected=TRUE,
                                        ...
                                        fileInput(
                                          inputId = "int_files",
                                          label = "Import interaction files",
                                          multiple = TRUE,
                                          accept=c('text/csv',
                                                   'text/comma-separated-values,text/plain',
                                                   '.csv'),
                       ...
                       tabsetPanel(type="tabs",
                                ...
                                Responses can extend beyond the time window in Surveda but the majority should fall within.",
                                   ...
                                   tabPanel("Contact Attempts and Lag",
                                            ...
                                            br(),
                                            plotOutput("contact_mode_tile_dispo", height=900),
                                            br(),
                                 ...
                                 divided by the total number of respondents contacted in that attempt.", align="center"),
                                   ...
                                   tabPanel("Errors",
                                            ...
                                            br(),
                                            downloadButton("errors_df2", "Download csv")),
...
server <- function(input, output, session){
  ...
  res_df <- reactive({
    ...
    numfiles = nrow(res_files)
    print(numfiles)
    res_df <- list()
    for (i in 1:numfiles){
      temp_df <- fread(input$res_files[[i, 'datapath']])
  ...
  output$result <- renderDT(head(res_df(), 100) %>% datatable(options = list(scrollX=TRUE)) %>% formatStyle(columns = c("section_order", "date", "modes", "sample_file", "mno"), "white-space" = "nowrap"))
  #-------------------------------------------------------------------------#


  #### Load Int Files ####
  int_df <- reactive({
    int_files <- input$int_files

    req(input$res_files, input$int_files)

    numfiles = nrow(int_files)
    print(numfiles)
    int_df <- list()
    for (i2 in 1:numfiles){
      temp_df2 <- fread(input$int_files[[i2, 'datapath']])
      # temp_df$mno <- "ASDF" #str_remove(temp_df$sample_file, "\\_[[:digit:]].*")
      temp_df2 <- temp_df2 %>% clean_names()
      int_df[[i2]] = temp_df2
    }
  ...
  output$int <- renderDT(head(int_df(), 100) %>% datatable(options = list(scrollX=TRUE)))
  #-------------------------------------------------------------------------#
  ...
  cont_resp <- reactive({
    ...
    temp_contact <- temp_int() %>% filter(action_type %in% c("Contact attempt", "Contacted", "Prompt", "Response")) %>%
      filter(action_data %!in% c("Enqueueing call")) %>%
  ...
  errors_df2 <- reactive({
    ...
    errors_df2 <- taco2 %>%
      ...
      mutate(prompt = ifelse(prompt %in% c("number of days exercise"), "physical activity", prompt)) %>%
  ...
  output$errors_df2 <- downloadHandler(
    filename = function(){
      paste0("errors_df2", Sys.Date(), ".csv")
    },
    content = function(file){
      write.csv(errors_df2(), file)
    }
  )
